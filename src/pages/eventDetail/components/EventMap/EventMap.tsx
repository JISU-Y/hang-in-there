...
const EventMap = ({ position }: EventMapProps) => {
  ...
  return (
    <>
      <ImageWrapper>
        <img
          src={`https://naveropenapi.apigw.ntruss.com/map-static/v2/raster-cors?w=1024&h=406&scale=2&center=${
            position.lng
          },${position.lat}&markers=type:d|size:mid|color:orange|pos:${
            position.lng
          }%20${position.lat}&level=12&X-NCP-APIGW-API-KEY-ID=${
            import.meta.env.VITE_NAVER_MAPS_CLIENT_ID
          }`}
        />
      </ImageWrapper>
